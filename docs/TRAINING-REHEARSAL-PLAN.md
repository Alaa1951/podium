...
تبقى حقول `AthleteProfile` و`requestedSeriesId` القديمة للتوافق والتفضيلات الأولية، ولا تحدد شريك الرياضي في مسابقة أخرى. الطلبات القديمة التي لا يمكن إثبات مسابقتها تبقى محفوظة خارج الإجراءات الجديدة. الأداة ترفض تكرار حساب رياضي في المصدر أو مقعدًا مرتبطًا بحساب دون مشاركة مقابلة؛ لا تصلح المصدر تلقائيًا. التسجيلات المستوردة غير المرتبطة بحساب قد تشترك في بريد المشتري: تنسخ كما هي مع إظهار عدد مجموعات البريد المشترك في المعاينة. تسجيل الدخول لا يربط عدة رياضيين في المسابقة نفسها بحساب واحد تلقائيًا؛ ربط هؤلاء يحتاج إثبات هوية منفصلًا.
...
التحقق المحلي يشمل مجموعة اختبارات الوحدة كاملة (847 اختبارًا)، و`type-check` و`lint` وبناء الإنتاج، واختبار MySQL على schema مؤقتة واختبار Chromium على شاشة موبايل. يتضمن ذلك حفظ اختيار Training أثناء التنقل، التسجيل بالحساب نفسه، وتعديل الهوية المشتركة، وحفظ المقاعد المستوردة التي تشترك في بريد تواصل واحد دون دمج هويات أصحابها. نتيجة بوابة GitHub تخص commit الإصدار نفسه، وتراجع قبل كل نشر.
