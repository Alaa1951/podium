...
export async function AnnouncementsPage(detailId?: string, compose = false) {
  ...
  if (detailId) {const item = sent[0]; return <PageShell title={item.title}><time dateTime={item.createdAt.toISOString()}>{new Intl.DateTimeFormat(locale,{dateStyle:"medium",timeStyle:"short"}).format(item.createdAt)}</time><p style={{whiteSpace:"pre-wrap",overflowWrap:"anywhere"}} dir="auto">{item.body}</p></PageShell>;}
  return <PageShell title={t("Announcements")} blurb={t("Write an announcement for the people you support.")} actions={<Link className="btn btn-secondary desktop-only" href={admin ? "/" : "/studio"}>{t("Back")}</Link>}>
    {compose ? <AnnouncementComposer admin={admin} studioId={user.studioId} studios={studios} readOnly={!!user.viewAs || !can(user, "announcements.manage")} /> : <><Link href={`${base}/new`} className="btn btn-primary mobile-only">{t("Send an announcement")}</Link><div className="desktop-only"><AnnouncementComposer admin={admin} studioId={user.studioId} studios={studios} readOnly={!!user.viewAs || !can(user,"announcements.manage")} /></div></>}
