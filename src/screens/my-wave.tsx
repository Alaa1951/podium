...
export default async function MyWavePage(detailId?: string) {
  ...
  if (grants.length === 0 && user.role === "competitor" && !detailId) {
    const team = await prisma.team.findFirst({where:{archivedAt:null,competitors:{some:{userId:user.id}}},orderBy:{series:{competitionDate:"desc"}},include:{series:true,waveRef:true}});
    return <div className="screen"><PlainHeader roleLabel={user.name ?? t("Competitor")} /><div className="screen-head"><h1>{t("My wave")}</h1></div>{team ? <article className="mobile-detail"><h2>{team.series.name}</h2><dl><dt>{t("Team")}</dt><dd>{team.name}</dd><dt>{t("Wave")}</dt><dd>{team.wave}</dd><dt>{t("Start time")}</dt><dd>{team.waveRef?.startTime ?? "—"}</dd><dt>{t("Status")}</dt><dd>{t(team.waveRef?.status ?? "pending")}</dd><dt>{t("Venue")}</dt><dd>{team.series.venue}</dd></dl><Link className="btn btn-primary" href="/me">{t("My team")}</Link></article> : <p className="notice">{t("No entry found for you yet.")}</p>}</div>;
  }
  if (grants.length === 0) {
  ...
  if (detailId) {
    const sheet = sheets.find((sheet) => sheet.rows.some((team) => team.id === detailId));
    if (!sheet) notFound();
    return <div className="screen"><PlainHeader roleLabel={user.name ?? t("Competitor")} /><ScoreGrid teams={sheet.rows} zones={sheet.zones} editBudget={0} budgetApplies={false} canEditAfterClose={user.role === "admin" || can(user,"scores.afterClose")} isAdmin={false} frozen={sheet.frozen} detailId={detailId} /></div>;
  }
