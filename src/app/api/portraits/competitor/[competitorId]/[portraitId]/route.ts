...
export async function GET(_req: Request, ctx: RouteContext<"/api/portraits/competitor/[competitorId]/[portraitId]">) {
  if (!portraitsEnabled()) return NextResponse.json({ error: "NOT_FOUND" }, { status: 404 });
  await requireUser();

  const { competitorId, portraitId } = await ctx.params;
  const portrait = await prisma.competitorPortrait.findFirst({
    where: { id: portraitId, competitorId },
