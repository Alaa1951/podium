...
describe("when everything is ready", () => {
  it("enters the pair, unpaid, as a sign-up", async () => {
    ...
    expect(data.competitors.create).toHaveLength(2);
    expect(data.competitors.create[0]).toMatchObject({ position: 1, userId: "a", email: "a@example.com" });
  });

  it("carries each athlete's own shirt size and studio onto their seat", async () => {
    await enterPairIfReady("a");
...
describe("the waiting list", () => {
  it("gives the pair a place when both signed up in time", async () => {
    expect(await enterPairIfReady("a")).toMatchObject({ waitlisted: false });
  });
  ...
  it("puts the pair on the list when the SECOND one was late", async () => {
    ...
    expect(mocks.createTeam.mock.calls[0][0].data.waitlistedAt).toBeInstanceOf(Date);
  });
...
describe("what must NOT produce a team", () => {
  ...
  it("an athlete with no partner", async () => {
    ...
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "NO_PARTNER" });
  });
  ...
  it("a partner who chose a DIFFERENT competition — one choosing is not consent", async () => {
    ...
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "PARTNER_CHOSE_ANOTHER" });
    expect(mocks.createTeam).not.toHaveBeenCalled();
  });
  ...
  it("an account still waiting for approval", async () => {
    ...
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "NOT_APPROVED" });
  });
  ...
  it("a finished, archived or inactive competition", async () => {
    ...
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "COMPETITION_CLOSED" });
  });

  it("a pair at two different levels", async () => {
    pair(athlete("a"), athlete("b", { profile: { division: "Pro" } }));
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "MIXED_LEVELS" });
  });
  ...
  it("a category the two genders cannot make", async () => {
    ...
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "CATEGORY_MISMATCH" });
  });

  it("a profile with no level or category yet", async () => {
    pair(athlete("a", { profile: { division: null } }));
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "INCOMPLETE_PROFILE" });
  });

  it("somebody a studio already entered by hand", async () => {
    mocks.findCompetitor.mockResolvedValue({ id: "existing" });
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "ALREADY_ENTERED" });
    expect(mocks.createTeam).not.toHaveBeenCalled();
  });
  ...
  it("a second simultaneous attempt, caught by the unique team number", async () => {
    ...
    mocks.createTeam.mockRejectedValue(Object.assign(new Error("dup"), { code: "P2002" }));
    expect(await enterPairIfReady("a")).toEqual({ entered: false, reason: "ALREADY_ENTERED" });
  });
  ...
  it("rethrows anything it does not recognise, rather than swallowing it", async () => {
    ...
    await expect(enterPairIfReady("a")).rejects.toThrow("database on fire");
  });
});
